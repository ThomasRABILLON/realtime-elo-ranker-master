...
export default function postMatchResult(baseUrl: string, adversaryA: string, adversaryB: string, result: MatchResult): Promise<Response> {
  return fetch(baseUrl + URL, {
    method: "POST",
    body: JSON.stringify({
      winner: result === MatchResult.LEFT_WIN ? adversaryA : result === MatchResult.RIGHT_WIN ? adversaryB : adversaryA,
      loser: result === MatchResult.LEFT_WIN ? adversaryB : result === MatchResult.RIGHT_WIN ? adversaryA : adversaryB,
